...
#  sqlite3 -cmd ".timeout 1000" $DBFILE "BEGIN TRANSACTION"
  ...
  for i in $LISTING; do
    EXISTS=`sqlite3 -cmd ".timeout 1000" $DBFILE "SELECT EXISTS(SELECT * FROM tvdupes WHERE fullpath=\"$i\")"`
    if [ $EXISTS -eq 0 ]; then
      # Season 1 format: S01E01     e.g. 3.Op.Reis.Backpack.S01E10.DUTCH.720p.HDTV.x264-WHAT3VAH
      # basedir -> 3.op.reis.backpack
      # season  -> s01
      season_info1=`expr match "$i" '.*\([Ss][0-9][0-9][Ee][0-9][0-9]*\)'`
      # Season 2 format: 2015-2016  e.g. RTL.Snowmagazine.2015-2016.E08.DUTCH.720p.HDTV.x264-P1ZZA
      # basedir -> rtl.snowmagazine
      # season  -> 2015-2016
      season_info2=`expr match "$i" '.*\(20[0-9][0-9][\-]20[0-9][0-9]*\)'`
      # season 3 format: 2015       e.g. Voetbal.Inside.2015.11.06.DUTCH.720p.HDTV.x264-CRAP
      # basedir -> voetbal.inside
      # season  -> 2015
      season_info3=`expr match "$i" '.*\(20[0-9][0-9]*\)'`

      if [ "$season_info1" ]; then
...
#        echo "dbg - SPECIALTAG $SPECIALTAG"
        ...
        ######### PART
        if [ `expr match "$i" '.*\.Part1\.*'` -gt 0 ]; then
          PART="01_Part1"
        elif [ `expr match "$i" '.*\.Part2\.*'` -gt 0 ]; then
          PART="02_Part2"
        else
          PART="00"
        fi
...
#        echo "dbg - DOWNVOTE $DOWNVOTE"
        ...
        POS=`awk -v a=$i -v b=$season_info1 'BEGIN{print index(a,b)}'`
        RELEASELENGTH=`expr length $i`
        TERMLENGTH=`expr length $season_info1`
...
proc_searchfordupes_with_pixels() {
  SQLPIXELS="$1"
  RELS=`sqlite3 -cmd ".timeout 1000" $DBFILE "SELECT DISTINCT seriebase,season,episode,part FROM tvdupes WHERE pixels='$SQLPIXELS'"`
  for REL in $RELS; do
    SERIEBASE=`echo $REL | cut -d"|" -f1`
    ...
    MATCHES=`sqlite3 -cmd ".timeout 1000" -header $DBFILE "select fullpath,pixels,source,codec,downvote,specialtag,created,internal,videobitrate,dynamicrange from tvdupes \
                                      where pixels='$SQLPIXELS' and seriebase='$SERIEBASE' and season='$SEASON' and episode='$EPISODE' and part='$PART' \
...
proc_searchfordupes() {
  RELS=`sqlite3 -cmd ".timeout 1000" $DBFILE "SELECT DISTINCT seriebase,season,episode,part FROM tvdupes"`
  for REL in $RELS; do
    SERIEBASE=`echo $REL | cut -d"|" -f1`
    SEASON=`echo $REL | cut -d"|" -f2`
    EPISODE=`echo $REL | cut -d"|" -f3`
    PART=`echo $REL | cut -d"|" -f4`
    MATCHES=`sqlite3 -cmd ".timeout 1000" -header $DBFILE "select fullpath,pixels,source,codec,downvote,specialtag,created,internal,videobitrate,dynamicrange from tvdupes \
                                      where seriebase='$SERIEBASE' and season='$SEASON' and episode='$EPISODE' and part='$PART' \
                                      order by pixels,source,codec,dynamicrange,downvote,specialtag,videobitrate DESC,created,internal"`
    NRMATCHES=`echo "$MATCHES" | wc -l`
    if [ $NRMATCHES -gt 2 ]; then
#      echo "$MATCHES"
      proc_cleanup "$MATCHES" "$1"
    fi
  done
}
...
proc_create_database() {
  sqlite3 -cmd ".timeout 1000" $DBFILE "DROP TABLE IF EXISTS tvdupes"
  sqlite3 -cmd ".timeout 1000" $DBFILE "CREATE TABLE tvdupes(fullpath TEXT UNIQUE, relgroup TEXT, seriebase TEXT, season TEXT, episode TEXT, part TEXT, internal TEXT, specialtag TEXT, downvote TEXT, codec TEXT, pixels TEXT, source TEXT, created TEXT, videobitrate INTEGER, dynamicrange TEXT)"
}
...
"
  ...
  ALLRECORDS=`sqlite3 -cmd ".timeout 1000" $DBFILE "select fullpath from tvdupes"`
  for i in $ALLRECORDS; do
#    local vbr=$(proc_get_video_bitrate_for $i)
#    echo "$i = $vbr"
    if [ ! -d $i ]; then
      echo "! WEG - $i"
      sqlite3 -cmd ".timeout 1000" $DBFILE "DELETE FROM tvdupes WHERE fullpath=\"$i\""
#    else
#      local vbr=$(proc_get_video_bitrate_for $i)
#      echo "$i = $vbr"
    fi
